...
from keras_frcnn.parse_test_data import get_data_test
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--path", dest="test_path", help="Path to test data.", default='dataset/png_TestIJCNN2013/gt_test.txt')
    parser.add_option("-n", "--num_rois", type="int", dest="num_rois",
                    help="Number of ROIs per iteration. Higher means more memory use.", default=32)
    parser.add_option("--config_filename", dest="config_filename", help=
                    "Location to read the metadata related to the training (generated when training).",
                    default="vggnet_config.pickle")
    parser.add_option("--network", dest="network", help="Base network to use. Supports vgg or fcnet.", default='vgg')
    ...
    ## use get_data function for loading train images to calculate mAP, else for testing use get_data_test function
    test_imgs, _, _ = get_data_test(options.test_path)
    # test_imgs = [s for s in all_imgs if s['imageset'] == 'test']      # for calculation of mAP
    ...
    for idx, img_data in enumerate(test_imgs):
        ...
        img_path = 'dataset/png_TestIJCNN2013/'
        ...
        for key in bboxes:
            ...
            for jk in range(new_boxes.shape[0]):
                ...
                textLabel = '{}: {:.3f}'.format(key, new_probs[jk])
        ...
        print("pred_bboxes:", pred_bboxs)
        cv2.imwrite('results_imgs/{}'.format(img_name), img)
        # print("gt boxes:", img_data['bboxes'])
        # cv2.imwrite('/results_imgs/{}'.format(img_name), img)

        
        # Uncomment for Calculate mAP using ground truth BBoxes
