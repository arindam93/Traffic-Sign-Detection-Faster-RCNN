...
from keras_frcnn.parse_data_to_dict import get_data
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--path", dest="train_path", help="Path to training data.", default="dataset/png_TrainIJCNN2013/gt_train.txt")

    parser.add_option("-o", "--parser", dest="parser", help="Parser to use. Only support simple", default="simple")

    parser.add_option("-n", "--num_rois", type="int", dest="num_rois", help="Number of RoIs to process at once.", default=32)

    parser.add_option("--network", dest="network", help="Base network to use. Support fcnet, vgg.", default='fcnet')

    parser.add_option("--hf", dest="horizontal_flips", help="Augment with horizontal flips in training. (Default=false).", default=False)

    parser.add_option("--vf", dest="vertical_flips", help="Augment with vertical flips in training. (Default=false).", default=False)

    parser.add_option("--rot", "--rot_90", dest="rot_90", help="Augment with 90 degree rotations in training. (Default=false).", default=False)
    ...
    parser.add_option("--config_filename", dest="config_filename", help="Location to store all the metadata related to the training (to be used when testing).", default="fcnet_config.pickle")

    parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='model_frcnn_fcnet.hdf5')
    ...
    ## all_imgs is a list of dictionaries
        ...
        while True:
            try:
                ...
                if len(pos_samples) > 0:
                    pos_samples = pos_samples[0]		# all pos samples
                else:
                    pos_samples = []
                ...
                if C.num_rois > 1:
                    ...
                    # replace: if sample is returned to pool for drawing repeatedly
                    try:
                ...
                losses[iter_num, 2] = loss_class[1]  # Detector classifier loss
                losses[iter_num, 3] = loss_class[2]  # Detector regression loss
                ...
                if iter_num == C.epoch_length:		# 600
